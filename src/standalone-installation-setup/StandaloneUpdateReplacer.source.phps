...
class StandaloneUpdateReplacer
{
    ...
    {
        $this->pikseraPath = dirname(dirname(dirname(__DIR__))) . DIRECTORY_SEPARATOR;
    ...
    {
        $steps_file = $this->newPikseraPath . DIRECTORY_SEPARATOR . 'replace_steps.json';
    ...
    {
        $this->deleteDirectoryRecursive($this->pikseraPath . 'vendor');
        $this->deleteDirectoryRecursive($this->pikseraPath . 'bootstrap ' . DIRECTORY_SEPARATOR . 'cache');
        $this->deleteDirectoryRecursive($this->pikseraPath . 'storage' . DIRECTORY_SEPARATOR . 'framework' . DIRECTORY_SEPARATOR . 'cache');
        $this->deleteDirectoryRecursive($this->pikseraPath . 'storage' . DIRECTORY_SEPARATOR . 'framework' . DIRECTORY_SEPARATOR . 'views');
        $this->deleteDirectoryRecursive($this->pikseraPath . 'userfiles' . DIRECTORY_SEPARATOR . 'cache');
        $this->deleteDirectoryRecursive($this->pikseraPath . 'userfiles' . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'admin-css');
    ...
    public function performFilesCopy($newFilesForCopy)
    {
        ...
        foreach ($newFilesForCopy as $newFile) {
            $copyIterator++;
            $newFileFolder = dirname($this->pikseraPath . $newFile['targetPath']);
            if (!is_dir($newFileFolder)) {
                mkdir_recursive($newFileFolder);
            }
            $target = $this->pikseraPath . $newFile['targetPath'];
